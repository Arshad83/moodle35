...
/**
 * This file contains main class for the course format mycustom
 *
 * @since     2.0
...
/**
 * Main class for the mycustom course format
 *
 * @since 2.0
 * @package format_mycustom
 * @copyright 2012 David Herney Bernal - cirano
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_mycustom extends format_base {
    ...
    protected function __construct($format, $courseid) {
        ...
        if (!self::$loaded && isset($section) && $courseid &&
                ($PAGE->pagetype == 'course-view-mycustom' || $PAGE->pagetype == 'course-view')) {
            self::$loaded = true;
            ...
            // mycustom format is always multipage.
            ...
            if ((!$canviewhidden && (!$sections[$realsection]->uservisible || !$sections[$realsection]->available))) {
                ...
                self::$byhiddenmsg = get_string('hidden_message', 'format_mycustom', $this->get_section_name($realsection));
    ...
    public function supports_ajax() {
        ...
        if (!isset($USER->mycustom_da)) {
            $USER->mycustom_da = array();
        }

        if (empty($COURSE)) {
            $disableajax = false;
        } else {
            $disableajax = isset($USER->mycustom_da[$COURSE->id]) ? $USER->mycustom_da[$COURSE->id] : false;
        }
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($foreditform && !isset($courseformatoptions['coursedisplay']['label'])) {
            $courseformatoptionsedit = array(
                ...
                'hidetabsbar' => array(
                    'label' => get_string('hidetabsbar', 'format_mycustom'),
                    'help' => 'hidetabsbar',
                    'help_component' => 'format_mycustom',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(
                ...
                'coursedisplay' => array(
                    'label' => new lang_string('coursedisplay', 'format_mycustom'),
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(
                            COURSE_DISPLAY_SINGLEPAGE => new lang_string('coursedisplay_single', 'format_mycustom'),
                            COURSE_DISPLAY_MULTIPAGE => new lang_string('coursedisplay_multi', 'format_mycustom')
                        )
                    ),
                    'help' => 'coursedisplay',
                    'help_component' => 'format_mycustom',
                ...
                'templatetopic' => array(
                    'label' => new lang_string('templatetopic', 'format_mycustom'),
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(
                            self::TEMPLATETOPIC_NOT => new lang_string('templetetopic_not', 'format_mycustom'),
                            self::TEMPLATETOPIC_SINGLE => new lang_string('templetetopic_single', 'format_mycustom'),
                            self::TEMPLATETOPIC_LIST => new lang_string('templetetopic_list', 'format_mycustom')
                        )
                    ),
                    'help' => 'templatetopic',
                    'help_component' => 'format_mycustom',
                ...
                'templatetopic_icons' => array(
                    'label' => get_string('templatetopic_icons', 'format_mycustom'),
     ...
     *
     * In case if course format was changed to 'mycustom', we try to copy
        ...
        if ($foreditform) {
            $sectionformatoptionsedit = array(
                'level' => array(
                    'default' => 0,
                    'type' => PARAM_INT,
                    'label' => get_string('level', 'format_mycustom'),
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(
                            0 => get_string('asprincipal', 'format_mycustom'),
                            1 => get_string('aschild', 'format_mycustom')
                        )
                    ),
                    'help' => 'level',
                    'help_component' => 'format_mycustom',
                ...
                'firsttabtext' => array(
                    'default' => get_string('index', 'format_mycustom'),
                    'type' => PARAM_TEXT,
                    'label' => get_string('firsttabtext', 'format_mycustom'),
                    'help' => 'firsttabtext',
                    'help_component' => 'format_mycustom',
                ),
                'fontcolor' => array(
                    'default' => '',
                    'type' => PARAM_RAW,
                    'label' => get_string('fontcolor', 'format_mycustom'),
                    'help' => 'fontcolor',
                    'help_component' => 'format_mycustom',
                ),
                'bgcolor' => array(
                    'default' => '',
                    'type' => PARAM_RAW,
                    'label' => get_string('bgcolor', 'format_mycustom'),
                    'help' => 'bgcolor',
                    'help_component' => 'format_mycustom',
                ),
                'cssstyles' => array(
                    'default' => '',
                    'type' => PARAM_RAW,
                    'label' => get_string('cssstyles', 'format_mycustom'),
                    'help' => 'cssstyles',
                    'help_component' => 'format_mycustom',
                )
            );
 ...
 * @return \core\output\inplace_editable
 */
function format_mycustoms_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            array($itemid, 'mycustom'), MUST_EXIST);
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_mycustom_replace_regularexpression {
