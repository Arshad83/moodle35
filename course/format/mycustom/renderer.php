...
/**
 * mycustom renderer logic implementation.
 *
 * @since 2.0
 * @package format_mycustom
...
/**
 * Basic renderer for mycustom format.
 *
 * @copyright 2012 David Herney Bernal - cirano
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_mycustom_renderer extends format_section_renderer_base {
    ...
    public function print_single_section_page($course, $sections, $mods, $modnames, $modnamesused, $displaysection) {
        ...
        if ($course->realcoursedisplay == COURSE_DISPLAY_MULTIPAGE) {
            ...
            if ((($thissection->visible && $thissection->available) || $canviewhidden) &&
                ...
                if ($this->_course->templatetopic == format_mycustom::TEMPLATETOPIC_NOT) {
                    echo $this->courserenderer->course_section_cm_list($course, $thissection, $displaysection);
                } else if ($this->_course->templatetopic == format_mycustom::TEMPLATETOPIC_LIST) {
        ...
        echo html_writer::start_tag('div', array('class' => 'single-section mycustom'));
                    ...
                    if ($formatoptions['level'] == 0 || count($tabs) == 0) {
                        ...
                        if (!is_array($tabs[$parentindex]->subtree)) {
                            ...
                            if ($parentformatoptions['firsttabtext']) {
                                ...
                                $firsttabtext = get_string('index', 'format_mycustom');
                            }
        ...
        if ($sections[$displaysection]->uservisible || $canviewhidden) {
            ...
            if ($course->realcoursedisplay != COURSE_DISPLAY_MULTIPAGE || $displaysection !== 0) {
                ...
                if ($this->_course->templatetopic == format_mycustom::TEMPLATETOPIC_NOT) {
                    echo $this->courserenderer->course_section_cm_list($course, $thissection, $displaysection);
                } else if ($PAGE->user_is_editing() || $this->_course->templatetopic == format_mycustom::TEMPLATETOPIC_LIST) {
                    echo $this->custom_course_section_cm_list($course, $thissection, $displaysection);
                }
        ...
        if ($PAGE->user_is_editing() && has_capability('moodle/course:update', $context)) {
            ...
            print_collapsible_region_start('move-list-box clearfix collapsible mform', 'course_format_mycustom_config_movesection',
                get_string('utilities', 'format_mycustom'), '', true);

            // Move controls.
            if ($canmove && !empty($movelisthtml)) {
                echo html_writer::start_div("form-item clearfix");
                    echo html_writer::start_div("form-label");
                        echo html_writer::tag('label', get_string('movesectionto', 'format_mycustom'));
                    ...
                    echo html_writer::start_div("form-description");
                        echo html_writer::tag('p', get_string('movesectionto_help', 'format_mycustom'));
                    echo html_writer::end_div();
                echo html_writer::end_div();
            }
            ...
            global $USER, $OUTPUT;
            if (isset($USER->mycustom_da[$course->id]) && $USER->mycustom_da[$course->id]) {
                $url->param('mycustom_da', 0);
                $textbuttondisableajax = get_string('enable', 'format_mycustom');
            } else {
                $url->param('mycustom_da', 1);
                $textbuttondisableajax = get_string('disable', 'format_mycustom');
            }

            echo html_writer::start_div("form-item clearfix");
                echo html_writer::start_div("form-label");
                    echo html_writer::tag('label', get_string('disableajax', 'format_mycustom'));
                echo html_writer::end_div();
                echo html_writer::start_div("form-setting");
                    echo html_writer::link($url, $textbuttondisableajax);
                echo html_writer::end_div();
                echo html_writer::start_div("form-description");
                    echo html_writer::tag('p', get_string('disableajax_help', 'format_mycustom'));
                echo html_writer::end_div();
            echo html_writer::end_div();

            // Duplicate current section option.
            if (has_capability('moodle/course:manageactivities', $context)) {
                $urlduplicate = new moodle_url('/course/format/mycustom/duplicate.php',
                                array('courseid' => $course->id, 'section' => $displaysection, 'sesskey' => sesskey()));

                $link = new action_link($urlduplicate, get_string('duplicate', 'format_mycustom'));
                $link->add_action(new confirm_action(get_string('duplicate_confirm', 'format_mycustom'), null,
                    get_string('duplicate', 'format_mycustom')));

                echo html_writer::start_div("form-item clearfix");
                    echo html_writer::start_div("form-label");
                        echo html_writer::tag('label', get_string('duplicatesection', 'format_mycustom'));
                    echo html_writer::end_div();
                    echo html_writer::start_div("form-setting");
                        echo $this->render($link);
                    echo html_writer::end_div();
                    echo html_writer::start_div("form-description");
                        echo html_writer::tag('p', get_string('duplicatesection_help', 'format_mycustom'));
                    echo html_writer::end_div();
                echo html_writer::end_div();
            }
    ...
    private function replace_resources ($section) {
        ...
        if (!empty($section->sequence)) {
            ...
            $objreplace = new format_mycustom_replace_regularexpression();
            ...
            foreach ($sectionmods as $modnumber) {
                ...
                if (!empty($availabilitytext)) {
                    $uniqueid = 'format_mycustom_winfo_' . time() . '-' . rand(0, 1000);
        ...
        if ($this->_course->templatetopic_icons == 0) {
            $summary = '<span class="mycustom_hideicons">' . $summary . '</span>';
        }
