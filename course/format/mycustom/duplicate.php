...
$PAGE->set_url('/course/format/mycustom/duplicate.php', array('courseid' => $courseid, 'section' => $section));
...
if (!empty($sectioninfo)) {
    ...
    $pbar = new progress_bar('mycustom_duplicate_bar', 500, true);
    $pbar->update_full(1, get_string('duplicating', 'format_mycustom'));
    ...
    $pbar->update_full(5, get_string('creating_section', 'format_mycustom'));
    ...
    $pbar->update_full(10, get_string('rebuild_course_cache', 'format_mycustom'));
    ...
    if (is_object($modinfo) && isset($modinfo->sections[$section])) {
        ...
        if (is_array($sectionmods)) {
            ...
            $k = 0;
            $pbar->update_full(40, get_string('progress_counter', 'format_mycustom', $dataprogress));
            foreach ($sectionmods as $modnumber) {
                ...
                $pbar->update_full($percent, get_string('progress_counter', 'format_mycustom', $dataprogress));
            }
        }
    } else {
        $pbar->update_full(100, get_string('progress_full', 'format_mycustom'));
    }

    $sectiontogo = $numnewsection;
} else {
    $sectiontogo = $section;
    echo get_string('error_nosectioninfo', 'format_mycustom');
